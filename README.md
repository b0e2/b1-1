...
| 상황 | 결과 |
| --- | --- |
| 유효한 캐시 있음 | 요청하지 않고 캐시로 렌더 |
| 캐시 없음·만료 → 요청 성공 | 새 응답으로 렌더하고 캐시 갱신 |
| 요청 실패 + 캐시 있음 | **오류 화면 대신 캐시로 렌더**하고 최신이 아닐 수 있음을 안내 |
| 요청 실패 + 캐시 없음 | 기존 403·404·기본 오류 문구와 error 화면 그대로 |

세 번째 갈래가 이 변경의 핵심입니다. 한도를 다 쓴 방문자에게 빈 오류 화면을 보여 주는 것보다,
조금 지난 목록이라도 보여 주고 **최신이 아닐 수 있다고 알리는 편이 낫습니다.**
`다시 시도` 버튼은 유효 기간과 무관하게 캐시를 건너뛰고 새 요청을 보냅니다.
그 요청마저 실패하면 다시 캐시로 되돌아가므로, 눌러서 손해 보는 경우가 없습니다.
...
상태에 늘어난 값도 `usedCache`와 `cachedAt` 둘뿐이고, 화면 상태 이름은 여전히
`loading` · `ready` · `error` · `empty` 넷입니다.
